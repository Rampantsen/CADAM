...
export function PromptView() {
  ...
  const ensureLocalConversation = useCallback(async () => {
    if (!isLocalApiEnabled) return;
    await localApiRequestJson<Conversation, Partial<Conversation>>(
  ...
  const { mutate: handleGenerate } = useMutation({
    mutationFn: async (content: Content) => {
      ...
      const conversation = isLocalApiEnabled
        ? await localApiRequestJson<Conversation, Partial<Conversation>>(
            '/api/v1/conversations',
            {
              method: 'POST',
            ...
            <div className="w-full max-w-3xl space-y-4 pb-12">
              ...
              >
                <TextAreaChat
                  ...
                  onBeforeUpload={ensureLocalConversation}
                />
              </SelectedItemsContext.Provider>
