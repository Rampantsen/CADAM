...
def call_genai_model(messages: list[dict], requested_model: str) -> str:
    settings = get_settings()
...
def build_model_content(model: str, code: str, raw_text: str) -> dict:
...
async def stream_parametric_placeholder(
    ...
    try:
        raw_text = await generate_parametric_code(
        ...
        assistant.content = build_model_content(request.model, code, raw_text)
